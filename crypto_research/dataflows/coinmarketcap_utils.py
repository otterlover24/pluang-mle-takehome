...
class CoinMarketCapAPI:
    ...
    def __init__(self, api_key: Optional[str] = None, use_sandbox: bool = False):
        ...
        self.headers = {"X-CMC_PRO_API_KEY": self.api_key, "Accept": "application/json"}
...
def get_crypto_fundamentals(ticker: str, api_key: Optional[str] = None) -> Dict:
    """
    ...
    if quote_data and "data" in quote_data:
        quote = (
            quote_data["data"].get(str(crypto_id), {}).get("quote", {}).get("USD", {})
        )
        fundamentals.update(
            {
        ...
        fundamentals.update(
            {
                ...
                "website": (
                    info.get("urls", {}).get("website", [""])[0]
                    if info.get("urls", {}).get("website")
                    else ""
                ),
                "technical_doc": (
                    info.get("urls", {}).get("technical_doc", [""])[0]
                    if info.get("urls", {}).get("technical_doc")
                    else ""
                ),
                "tags": info.get("tags", []),
...
def format_crypto_data_for_agents(
    ticker: str, start_date: str, end_date: str, api_key: Optional[str] = None
) -> Dict:
    """
