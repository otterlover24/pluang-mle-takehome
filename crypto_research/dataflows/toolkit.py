...
from ..research_configs import get_config, DATA_DIR
...
class Toolkit:
    def __init__(self, config: Optional[Dict] = None):
        """Initialize the toolkit with configuration."""
        if config is None:
            config = get_config()
    ...
    def get_crypto_market_sentiment(
        ...
        try:
            ...
            sentiment_report += (
                f"- 24h Change: {fundamentals['percent_change_24h']:.2f}%\n"
            )
            sentiment_report += (
                f"- 7d Change: {fundamentals['percent_change_7d']:.2f}%\n"
            )
            sentiment_report += (
                f"- 30d Change: {fundamentals['percent_change_30d']:.2f}%\n\n"
            )
            ...
            sentiment_report += (
                f"- Total Market Cap: ${market_metrics['total_market_cap']:,.0f}\n"
            )
            sentiment_report += (
                f"- Bitcoin Dominance: {market_metrics['bitcoin_dominance']:.2f}%\n"
            )
            sentiment_report += (
                f"- Total 24h Volume: ${market_metrics['total_volume_24h']:,.0f}\n"
            )
            sentiment_report += (
                f"- DeFi Market Cap: ${market_metrics['defi_market_cap']:,.0f}\n"
            )
    ...
    def get_crypto_fundamentals_report(
        ...
        try:
            ...
            if fundamentals["technical_doc"]:
                report += (
                    f"- Technical Documentation: {fundamentals['technical_doc']}\n"
                )
            ...
            report += (
                f"- Circulating Supply: {fundamentals['circulating_supply']:,.0f}\n"
            )
            ...
            if fundamentals["platform"]:
                ...
                report += (
                    f"- Platform: {fundamentals['platform'].get('name', 'Unknown')}\n"
                )
    ...
    def get_crypto_price_data_window(
        ...
        try:
            ...
            # DEBUG
            print(
                f"Retrieved historical quotes for {symbol} from {start_date} to {end_date}"
            )
            print(df.head())
        ...
        while curr_date_dt <= start_date:
            ...
            fetch_result = fetch_top_from_category(
                ...
                data_path=(
                    ...
                    if os.path.exists(os.path.join(DATA_DIR, "reddit_data"))
                    else None
                ),
            )
    ...
    def get_crypto_market_overview(
        ...
        try:
            ...
            overview += (
                f"- Altcoin Market Cap: ${metrics['altcoin_market_cap']:,.0f}\n\n"
            )
            ...
            overview += (
                f"- Stablecoin Market Cap: ${metrics['stablecoin_market_cap']:,.0f}\n"
            )
            overview += (
                f"- Stablecoin 24h Volume: ${metrics['stablecoin_volume_24h']:,.0f}\n\n"
            )
            ...
            total_cap_change = (
                ...
                / metrics["total_market_cap_yesterday"]
                * 100
            )
            volume_change = (
                (metrics["total_volume_24h"] - metrics["total_volume_24h_yesterday"])
                / metrics["total_volume_24h_yesterday"]
                * 100
            )
